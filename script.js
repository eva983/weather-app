...
const weatherDiv = document.querySelector(".rest");
var isCelsius = true;
...
function showWeather(data) {
  ...
  if (data.cod === "404") {
    ...
    const parts = date_time.split(",");
    const weekday = parts[0];
    const date = parts[1];
    const time = parts[2];

    const date_div = document.querySelector(".date-container");
    ...
    date_div.innerHTML = `
	<h3>${weekday}</h3>
	<div class="date_cont">${date}<br>${time}</div>`;

    city.innerHTML = `<svg class="loc-icon"  xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-geo-fill" viewBox="0 0 16 16">
